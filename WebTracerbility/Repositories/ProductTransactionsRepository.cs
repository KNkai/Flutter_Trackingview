...
namespace BidPhar_services.Repositories
{
    public class ProductTransactionsRepository : IProductTransactions
    {
        ...
        public async Task<int> DeleteProductTransactionByIdProduct(string id)
        {
            var productTransaction = _db.ProductTransactions.Where(data => data.IdProduct == id).ToList();
            foreach (var item in productTransaction)
            {
                _db.ProductTransactions.Remove(item);
                await _db.SaveChangesAsync();
            }
            return 1;
        }
